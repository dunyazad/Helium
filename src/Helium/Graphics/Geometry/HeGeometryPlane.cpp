...
namespace ArtificialNature {
	...
	HeGeometryPlane::HeGeometryPlane(const string& name, float columnLength, float rowLength, unsigned int colomns, unsigned int rows, HePlaneType type)
		: HeGeometry(name), columnLength(columnLength), rowLength(rowLength), columns(colomns), rows(rows), type(type)
	{
	}

	HeGeometryPlane::~HeGeometryPlane()
	{
	}

	void HeGeometryPlane::Initialize()
	{
	...
	void HeGeometryPlane::Terminate()
	{
