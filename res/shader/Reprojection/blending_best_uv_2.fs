...
void main() {
	...
	float uvCenterSecondDistance = FLT_MAX;
	int uvCenterSecondFrame = 0;
	vec2 uvCenterSecondFrameUV;
		...
		if(IsInsideOfFOV(uv)) {
			...
			if(abs(angle) < PI / 2) {
				...
				if(uvDistance < uvCenterDistance) {
					uvCenterSecondDistance = uvCenterDistance;
					uvCenterSecondFrame = uvCenterFrame;
					uvCenterSecondFrameUV = uvCenterFrameUV;

					uvCenterDistance = uvDistance;
					uvCenterFrame = i;
					uvCenterFrameUV = uv;
				} else if(uvDistance < uvCenterSecondDistance) {
					uvCenterSecondDistance = uvDistance;
					uvCenterSecondFrame = i;
					uvCenterSecondFrameUV = uv;
				}
			}
		}
	}
	
	vec4 textureA = texture(textureArray, vec3(uvCenterFrameUV.x, uvCenterFrameUV.y, uvCenterFrame));
	vec4 textureB = texture(textureArray, vec3(uvCenterSecondFrameUV.x, uvCenterSecondFrameUV.y, uvCenterSecondFrame));
	float ratio = 0.5;
	FragColor = mix(textureA, textureB, ratio);
}
